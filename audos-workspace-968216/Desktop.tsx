...
// Google Search Console site verification. The served <head> is generated by
// the platform compiler at publish time (workspace files contain no index.html),
// so the tag is injected at bundle load as early as possible. For guaranteed
// raw-HTML placement it must also be added to the workspace-level
// custom_head_html tracking setting on the platform side.
if (typeof document !== 'undefined' && !document.querySelector('meta[name="google-site-verification"]')) {
  ...
  gscMeta.content = 'gHHbhfT_sP_5ESt-BzKqOOP4Kki8c_VA4K5YNYQbwWE';
  document.head.prepend(gscMeta);
}
...
// ─── Admin-only apps ── hidden from the dock and deep links unless the signed-in
// email is on the allowlist. Keep ADMIN_APP_ALLOWED_EMAILS in sync with
// ADMIN_EMAILS in apps/AdminPortal/App.tsx (which gates the app's content).
const ADMIN_ONLY_APP_IDS = new Set(['admin']);
const ADMIN_APP_ALLOWED_EMAILS = ['altanativez.wealth@gmail.com'];

function getSessionEmailForAppVisibility(): string | null {
  try {
    const w = window as any;
    const spaceId = w.__SPACE_ID__ || w.__APP_ID__ || '';
    let raw = spaceId ? localStorage.getItem(`space_session_${spaceId}`) : null;
    if (!raw) {
    ...
    if (raw) {
      ...
      if (typeof s.email === 'string' && s.email.includes('@')) return s.email.toLowerCase().trim();
    }
  } catch { /* storage unavailable */ }
  return null;
}

function isAppVisibleToSession(appId: string, mode?: string): boolean {
  if (!ADMIN_ONLY_APP_IDS.has(appId)) return true;
  if (mode === 'entrepreneur') return true; // founder studio preview sees everything
  const email = getSessionEmailForAppVisibility();
  return !!email && ADMIN_APP_ALLOWED_EMAILS.some(e => e.toLowerCase().trim() === email);
}
...
export default function SpaceDesktop({
  ...
  // Apps the current visitor is allowed to see (admin-only apps are filtered
  // out for everyone except allowlisted admin emails / the founder preview).
  // sessionId is a dependency so this recomputes once the email gate resolves.
  const visibleApps = useMemo(
    () => config.apps.filter(app => isAppVisibleToSession(app.id, mode)),
    [config.apps, mode, sessionId]
  );
  ...
  useEffect(() => {
    ...
    if (deepLinkId) {
      const matchingApp = visibleApps.find(
  ...
  useEffect(() => {
    const handleOpenApp = (event: CustomEvent) => {
      ...
      if (appId && isAppVisibleToSession(String(appId), mode)) {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-2xl text-left transition-all border ${
                      isActive
                        ? 'bg-[var(--space-surface-card)] border-[var(--space-brand-primary-500)]/40 shadow-[0_4px_14px_color-mix(in_srgb,var(--space-shell-shadow-strong)_45%,transparent)]'
                        : 'bg-[var(--space-surface-card)] border-[var(--space-border-default)] shadow-[0_1px_4px_color-mix(in_srgb,var(--space-shell-shadow)_30%,transparent)] hover:shadow-[0_4px_14px_color-mix(in_srgb,var(--space-shell-shadow-strong)_40%,transparent)] hover:-translate-y-px'
                    }`}
      ...
      return (
        <div className="flex items-center justify-between gap-3 px-4 py-3 flex-shrink-0 border-b border-[var(--space-border-subtle,var(--space-surface-muted))]">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium flex-shrink-0 bg-[var(--space-brand-highlight)] text-[var(--space-text-on-highlight)] hover:bg-[var(--space-brand-highlight-700)] transition-colors shadow-[0_4px_14px_color-mix(in_srgb,var(--space-brand-highlight)_35%,transparent)]"
  ...
  return (
    <>
      ...
      <link
        href={`https://fonts.googleapis.com/css2?family=${encodeURIComponent(runtimeTheme.themeTokens.typography?.headingFont || 'Plus Jakarta Sans')}:wght@300;400;500;600;700;800&display=swap`}
        rel="stylesheet"
      />
      ...
      <div className="fixed inset-0 flex overflow-hidden" style={rootStyle}>
        ...
        <aside
          className={`z-50 flex-shrink-0 flex flex-col min-h-0 overflow-hidden transition-all duration-300 ease-in-out max-md:fixed max-md:inset-y-0 max-md:left-0 max-md:w-72 max-md:max-w-[85vw] max-md:shadow-[0_22px_56px_color-mix(in_srgb,var(--space-shell-shadow-strong)_55%,transparent)] ${
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0">
          ...
          <div className="flex-1 flex min-w-0 min-h-0 md:gap-3 md:p-3">
            ...
            <main
              className={`flex-1 flex-col min-w-0 min-h-0 overflow-hidden bg-[var(--space-surface-card)] md:rounded-2xl md:shadow-[0_2px_16px_color-mix(in_srgb,var(--space-shell-shadow)_40%,transparent),0_1px_3px_color-mix(in_srgb,var(--space-shell-shadow)_25%,transparent)] ${
            ...
            {activePanelId && (
              <section
                className={`flex-col min-h-0 overflow-hidden bg-[var(--space-surface-card)] md:flex md:rounded-2xl md:shadow-[0_2px_16px_color-mix(in_srgb,var(--space-shell-shadow)_40%,transparent),0_1px_3px_color-mix(in_srgb,var(--space-shell-shadow)_25%,transparent)] max-md:flex-1 max-md:min-w-0 ${
